...
class REVGame extends GameBase {
  ...
  void render() {
    ...
    switch (state) {
      ...
      case REV_PLAYING:
        ...
        if (mode == REV_ONLINE && !flipAnimating) revReceive();
        renderer.drawGame();
        break;
      case REV_GAMEOVER:
        if (mode == REV_ONLINE) revReceive();
        renderer.drawGame();
        break;
      case REV_HOWTO:
  ...
  void onKeyPressed() {
    if (state == REV_LOBBY && lobbyState == LOBBY_JOINING) {
  ...
  void onEscape() {
    switch (state) {
      ...
      case REV_PLAYING:
      case REV_GAMEOVER:
        if (mode == REV_ONLINE) network.stop();
        state = REV_MENU;
        particles.clear();
        break;
      case REV_HOWTO:
  ...
  void handlePlayClick() {
    ...
    if (mode == REV_ONLINE && currentPlayer != playerRole) return;

    int[] cell = renderer.getCellUnderMouse();
    if (cell == null) return;

    if (board.getFlips(cell[0], cell[1], currentPlayer).size() == 0) return;
    if (mode == REV_ONLINE) network.send("MOVE:" + cell[0] + ":" + cell[1]);
  ...
  void handleGameOverClick() {
    ...
    if (mouseX > rx - bw / 2 && mouseX < rx + bw / 2 && mouseY > ry - bh / 2 && mouseY < ry + bh / 2) {
      if (mode == REV_ONLINE) network.send("REMATCH");
      startPlay(mode);
    }
      ...
      if (data.startsWith("MOVE:")) {
        ...
        if (parts.length == 3) {
          try {
            ...
            if (board.getFlips(r, c, currentPlayer).size() > 0) {
              executeMove(r, c);
            }
          } catch (Exception e) {}
        }
