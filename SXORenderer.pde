...
class SXORenderer {
  ...
  void drawLobby() {
    ...
    switch (game.lobbyState) {
      case SXO_LOBBY_CHOOSE:
        ...
        drawButton(CANVAS_W / 2, 320, "Host Game", SXO_COLOR_ACTIVE);
        drawButton(CANVAS_W / 2, 400, "Join Game", SXO_COLOR_O);
        drawButton(CANVAS_W / 2, 520, "Back", color(120));
        break;
      ...
      case SXO_LOBBY_HOSTING:
        ...
        text(game.hostRoomCode, CANVAS_W / 2, 290);
        textSize(14);
        fill(150);
        ...
        drawButton(CANVAS_W / 2, 520, "Cancel", color(120));
        break;
      ...
      case SXO_LOBBY_JOINING:
        ...
        drawCodeInput(CANVAS_W / 2, 290, game.roomCode);
        textSize(12);
        fill(150);
        text("Type the 8-character hex code and press Enter", CANVAS_W / 2, 350);
        if (game.network.joining) {
        ...
        } else if (game.network.joinError.length() > 0) {
          textSize(16);
          fill(SXO_COLOR_X);
          text(game.network.joinError, CANVAS_W / 2, 390);
          if (game.roomCode.length() == 8) {
            drawButton(CANVAS_W / 2, 430, "Retry", SXO_COLOR_ACTIVE);
          }
        } else if (game.roomCode.length() == 8) {
          drawButton(CANVAS_W / 2, 430, "Connect", SXO_COLOR_ACTIVE);
        } else {
          noStroke();
          fill(80, 80, 100, 80);
  ...
  void drawCodeInput(float x, float y, String code) {
